...
import { publicFetch, setToken, setProvider, ApiError, oauthLogin } from "@/lib/api";
import { startLichessOAuth } from "@/lib/lichess";
...
export default function LoginPage() {
  ...
  const [oauthLoading, setOauthLoading] = useState<"google" | "lichess" | null>(null);
    ...
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        await oauthLogin({
          ...
          username: userInfo.name?.replace(/\s+/g, "").toLowerCase() ?? userInfo.email.split("@")[0],
        });
        router.push("/home");
      } catch {
              ...
              <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 h-10 rounded-[var(--radius-sm)] border-2 border-border bg-surface flex items-center justify-center gap-2 text-sm font-medium text-text hover:border-primary hover:bg-surface-alt transition-all duration-200 cursor-pointer disabled:opacity-50"
                  >
                    {oauthLoading === "google" ? (
                  ...
                  <button
                    type="button"
                    onClick={() => { setOauthLoading("lichess"); startLichessOAuth(); }}
                    disabled={!!oauthLoading}
                    className="flex-1 h-10 rounded-[var(--radius-sm)] border-2 border-border bg-surface flex items-center justify-center gap-2 text-sm font-medium text-text hover:border-primary hover:bg-surface-alt transition-all duration-200 cursor-pointer disabled:opacity-50"
                  >
                    {oauthLoading === "lichess" ? (
                      <div className="w-4 h-4 border-2 border-primary border-t-transparent rounded-full animate-spin" />
                    ) : (
                      <svg className="w-4 h-4" viewBox="0 0 50 50" fill="currentColor">
                        <path d="M33.67 5.37 28.5 19.5H36L19 44.63l3-14.13H15l8.37-25.13z"/>
                      </svg>
                    )}
                    Lichess
                  </button>
